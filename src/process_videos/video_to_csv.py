...
def video_to_csv(video_folder_path: Path, raw_frames_folder_path: Path, flip_image: bool):
    ...
    for video_file_path in video_folder_path.glob('*.mp4'):
        ...
        result_csv_filename = raw_frames_folder_path / \
            f"{video_file_path.name.replace('.mp4', '_')}{current_time}_raw.csv"
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened() and success:
                ...
                csv_writer.read_data(
                    data=results.pose_landmarks, timestamp=cap.get(cv2.CAP_PROP_POS_MSEC))
...
def testing():
    video_folder_path = Path(
        r'C:\Users\Max\Documents\Master Würzburg\Machine Learning\final-project-getting-started\demo_data')
