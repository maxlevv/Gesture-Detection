...
def scale_to_body_size_and_dist_to_camera(vector_to_scale: np.array, window_df: pd.DataFrame):
    ...
    hip_mid_point = np.array([
        (window_df.iloc[0, window_df.columns.get_loc('left_hip_x')] +
         window_df.iloc[0, window_df.columns.get_loc('right_hip_x')]) / 2,
        (window_df.iloc[0, window_df.columns.get_loc('left_hip_y')] +
         window_df.iloc[0, window_df.columns.get_loc('right_hip_y')]) / 2
    ])
    shoulder_mid_point = np.array([
        (window_df.iloc[0, window_df.columns.get_loc('left_shoulder_x')] +
         window_df.iloc[0, window_df.columns.get_loc('right_shoulder_x')]) / 2,
        (window_df.iloc[0, window_df.columns.get_loc('left_shoulder_y')] +
         window_df.iloc[0, window_df.columns.get_loc('right_shoulder_y')]) / 2
    ])
...
def calc_differences(df: pd.DataFrame, preproc_params: Preprocessing_parameters) -> Tuple[np.array, pd.DataFrame]:
    ...
    df_for_diff = extract_features(
        df, preproc_params.mediapipe_columns_for_diff)
    ...
    if preproc_params.difference_mode == 'one':
        ...
        for i in range(num_samples):
            ...
            # apply scaling
            X[i, :] = scale_to_body_size_and_dist_to_camera(
                X[i, :], df.loc[i * num_shifts: i * num_shifts + num_timesteps - 1, :])
...
def correct_angle_boundary_diff(diff_np: np.array):
    ...
    diff_np[np.where(diff_np > np.pi)[0]] = diff_np[np.where(
        diff_np > np.pi)[0]] - 2 * np.pi
    diff_np[np.where(diff_np < -np.pi)[0]
            ] = diff_np[np.where(diff_np < -np.pi)[0]] + 2 * np.pi
...
def scale_angle_diff(angle_diff_np: np.array, window_start_index: int, num_timesteps: int, df, side: str):
    if side == 'right':
        r = df.loc[window_start_index: window_start_index +
                   num_timesteps - 1, 'right_forearm_r'].to_numpy()
    elif side == 'left':
        r = df.loc[window_start_index: window_start_index +
                   num_timesteps - 1, 'left_forearm_r'].to_numpy()

    r_mid = (r[1:] + r[:-1]) / 2
    r_mid_scaled = scale_to_body_size_and_dist_to_camera(
        r_mid*0.5, df.iloc[window_start_index: window_start_index + num_timesteps - 1, :])
    factor = np.zeros_like(r_mid_scaled)
    factor = np.power(r_mid_scaled, 6) * 100
    factor[np.where(r_mid_scaled > 1)
           ] = r_mid_scaled[np.where(r_mid_scaled > 1)]
    transformed_angle_diff = angle_diff_np * factor

    # set a max value
    caped_angle_diff = np.min(
        np.c_[transformed_angle_diff, np.ones_like(transformed_angle_diff)*10], axis=1)
    caped_angle_diff = np.max(
        np.c_[caped_angle_diff, np.ones_like(caped_angle_diff)*-10], axis=1)
...
def cumulative_sum(df: pd.DataFrame, preproc_params: Preprocessing_parameters, X) -> Tuple[np.array, pd.DataFrame]:
    ...
    if 'right_forearm_angle' in df_for_sum.columns:
        ...
        df_for_sum.drop(
            ['right_forearm_angle', 'left_forearm_angle'], axis=1, inplace=True)
    ...
    for i in range(num_samples):
        ...
        window_np = data[window_start_index: window_start_index +
                         num_timesteps, :]
        diff_window_np = window_np[1:, :] - window_np[:-1, :]

        if angle_involved:
            right_forearm_angle_diff = calc_angle_diff(
                right_forearm_angle, window_start_index, num_timesteps, df, side='right')
            left_forearm_angle_diff = calc_angle_diff(
                left_forearm_angle, window_start_index, num_timesteps, df, side='left')

            diff_window_np = np.c_[
                diff_window_np, right_forearm_angle_diff, left_forearm_angle_diff]
        ...
        X[i, :-2] = scale_to_body_size_and_dist_to_camera(
            X[i, :-2], df.loc[i * num_shifts: i * num_shifts + num_timesteps - 1, :])
...
def calc_forearm_angle(df: pd.DataFrame):
    ...
    angle_np[np.where(angle_np < 0)[0]] = angle_np[np.where(
        angle_np < 0)[0]] + 2 * np.pi
...
def shoulder_wrist_difference(df: pd.DataFrame, preproc_params: Preprocessing_parameters, X):
    ...
    for i in range(num_samples):
        ...
        x_shoulder_elbow_diff = np.subtract(
            shoulder_x[selection_index], elbow_x[selection_index])
        x_elbow_wrist_diff = np.subtract(
            elbow_x[selection_index], wrist_x[selection_index])

        y_shoulder_elbow_diff = np.subtract(
            shoulder_y[selection_index], elbow_y[selection_index])
        y_elbow_wrist_diff = np.subtract(
            elbow_y[selection_index], wrist_y[selection_index])

        right_dist = np.sqrt(np.power(x_shoulder_elbow_diff[0], 2) + np.power(y_shoulder_elbow_diff[0], 2)) + \
            np.sqrt(
                np.power(x_elbow_wrist_diff[0], 2) + np.power(y_elbow_wrist_diff[0], 2))

        left_dist = np.sqrt(np.power(x_shoulder_elbow_diff[1], 2) + np.power(y_shoulder_elbow_diff[1], 2)) + \
            np.sqrt(
                np.power(x_elbow_wrist_diff[1], 2) + np.power(y_elbow_wrist_diff[1], 2))

        X[i, -2] = right_dist
        X[i, -1] = left_dist

        X[i, -2:] = scale_to_body_size_and_dist_to_camera(
            X[i, -2:], df.loc[selection_index: selection_index, :])
...
def determine_label_from_ground_truth_vector(ground_truth_df: pd.DataFrame, num_timesteps: int,
    ...
    if 'rotate_left' in Labels.get_label_list():
        rotate_left_one_hot = np.zeros_like(
            data[0*num_shifts + num_timesteps-6, :])
        rotate_left_one_hot[Labels.rotate_left.value] = 1

    for i in range(num_samples):

        middle_one_hot = data[i*num_shifts + num_timesteps -
                              (int(np.ceil(num_timesteps/2)) + 1), :]
        # vom vorletzten im window ablesen
        second_to_last_one_hot = data[i*num_shifts + num_timesteps-2, :]
        # vom letzten im window ablesen
        last_one_hot = data[i*num_shifts + num_timesteps-1, :]
...
def create_X(df: pd.DataFrame, preproc_params: Preprocessing_parameters, verbose=False) -> Tuple[np.array, pd.DataFrame]:
    ...
    num_features = len(preproc_params.mediapipe_columns_for_sum) * \
        sum(preproc_params.summands_pattern) + 2
    ...
    if preproc_params.difference_mode:
        print(
            f'{bcolors.FAIL}WARNING: difference mode is deactivated, needs refactoring{bcolors.ENDC}')
    ...
    if preproc_params.summands_pattern:
        ...
        raise RuntimeError(
            'summands_pattern needed, implementation for diff needed')
    ...
    cumsum_column_names = [
        column_cumsum_name + f"_{str(i)}" for column_cumsum_name in column_cumsum_names for i in range(sum(preproc_params.summands_pattern))]
    shoulder_wrist_column_names = [
        "right_shoulder_elbow_wrist_dist", "left_shoulder_elbow_wrist_dist"]
    X_df = pd.DataFrame(data=X, columns=cumsum_column_names +
                        shoulder_wrist_column_names)
