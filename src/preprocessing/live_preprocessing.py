...
class LiveDfGenerator:
    ...
    def to_df(self):
        frames = pd.DataFrame(
            self.frame_list, columns=self.column_names, index=self.timestamps)
    ...
    def resample(cls, df: pd.DataFrame):
        ...
        new_index = pd.timedelta_range(
            old_index.min(), old_index.max(), freq='33ms')
    ...
    def reset_index(cls, df):
        df = df.reset_index()
        df.columns = [column.replace('index', 'timestamp')
...
def test_resample():
    df = pd.read_csv(Path(
        r'..\..\data\raw_frames\test\2022-03-08 08-25-04_2022-03-08_08-36-53_raw.csv'))
    df = df.loc[:5, :]
