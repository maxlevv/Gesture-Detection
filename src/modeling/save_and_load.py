...
def save_run(save_runs_folder_path: Path, run_group_name, neural_net: FCNN, author: str, data_file_name: str,
    ...
    meta_data = MetaData.from_neural_net(
        neural_net, author, data_file_name, lr, batch_size, epochs, num_samples, description, name)
    save_folder_path = save_meta_json_and_weights(
        neural_net, meta_data, save_runs_folder_path, run_group_name)
...
def save_meta_json_and_weights(neural_net: FCNN, meta_data: MetaData, save_runs_folder_path: Path, run_group_name: str):
    ...
    meta_data.run_number = len(
        [item for item in run_group_folder_path.iterdir()])
    run_folder_path = run_group_folder_path / \
        meta_data.generate_save_run_folder_name()
