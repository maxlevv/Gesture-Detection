...
def check_gradient(func: Callable, gradient: Callable, x_test: np.array):
    ...
    eps = 1e-4
    for i in range(n):
        num_grad[0, i] = (func(x_test + eps * np.identity(n)[:, i].reshape(-1, 1)) -
                          func(x_test - eps * np.identity(n)[:, i].reshape(-1, 1))) / (2 * eps)
...
def check_gradient_of_neural_net(net: FCNN, example_X: np.array, example_Y_g: np.array, verbose: bool = False):
    ...
    for w_index in range(len(net.W)):
        ...
        J = get_loss_func_only_dependent_one_weight_matrix(
                    net, example_X, example_Y_g, w_index)

        d_J = get_loss_gradient_only_dependent_one_weight_matrix(
                    net, example_X, example_Y_g, w_index)
    ...
    if verbose:
        print_gradients_to_compare(num_grads, net_grads)

    return np.array([grad_bool]).all()


def grad_check_test():
    def func(x): return 2*x[0]
    def g_x(x): return np.array([2, 0])
    x_test = np.array([5, 0])
